...
class TimeStats(object):
	...
	def export_tags_counts_graphs(self, folder, tags=None):
		if not os.path.exists(folder): os.makedirs(folder)
		...
		for username in times.keys():
			...
			if tags:
				tags_list = [tag for tag in tags[username].keys() if tag in tags]
			else:
				tags = tags[username].keys()
	...
	def export_tags_points_graphs(self, folder, tags=None):
		if not os.path.exists(folder): os.makedirs(folder)
